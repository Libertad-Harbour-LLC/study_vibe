...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image1.png)
...
2. Стоимость вывода (inference) сильно положительно коррелирует с длиной контекста. Чем длиннее контекст, тем больше растёт стоимость вывода — почти линейно, а иногда даже сверхлинейно. Для одного вызова 8K токенов и 200K токенов находятся в совершенно разных диапазонах цены и задержки, и у длинного контекста гораздо более высокий стоимостный порог.
   ...
   ![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image2.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image3.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image4.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image5.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image6.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image7.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image8.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image9.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image10.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image11.png)
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image12.png)

Традиционный RAG работает, находя текстовые фрагменты, похожие на вопрос, что похоже на выбор нескольких наиболее релевантных абзацев из груды материала. Это хорошо работает для прямого поиска фактов. Но если вопрос требует связать несколько документов и объединить разные подсказки, производительность падает.

Например, врач может спросить: «На основе этих случаев и новейших рекомендаций по лечению, как нам оценить пользу и риски определённого препарата для пожилых пациентов?» Или проектная команда может спросить: «Если посмотреть на документы требований, записи ревью и отчёты о проблемах в продакшене за прошедшие два года, какая часть нашей архитектуры системы отказывает чаще всего?» Вопросы вроде этих — не о нахождении одного предложения. Они требуют выявления людей, объектов, событий и отношений, разбросанных по нескольким материалам, и формирования целостной картины.

Graph RAG строит эту картину проактивно. Система использует большую модель для выявления из текста ключевых сущностей, таких как люди, организации, функциональные модули, события и данные, вместе с их отношениями, такими как причинность, зависимость, изменение и противоречие. Затем она строит сеть знаний, которая растёт по мере добавления материала. Через автоматическую группировку тесно связанные сущности и отношения организуются в темы, и каждую тему можно заранее суммировать. Когда пользователь задаёт вопрос, система больше не ищет только текстовые фрагменты, которые выглядят похоже. Она сначала находит наиболее релевантные сущности и локальную структуру графа, расширяется через связанные тематические группы, а затем передаёт LLM путь анализа, описания узлов и исходные фрагменты вместе для рассуждения.

В рамках этого подхода Graph RAG и традиционный RAG дополняют друг друга. Традиционный RAG остаётся сильным для детальных вопросов, ответы на которые можно найти за один шаг. Graph RAG ближе к тому, как мыслит исследователь-человек: сначала организовать общую структуру и темы, затем заполнить доказательствами и в конце выдать вывод с логикой и условиями. Существующие сравнения показывают, что в задачах многошагового рассуждения Graph RAG часто охватывает больше критического содержимого и даёт более широкую перспективу. Гибкое сочетание двух подходов часто лучше, чем использование только одного.

## 7.2 Мультимодальный RAG

Связанные исследования:

- https://arxiv.org/pdf/2502.08826

![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image13.png)

Данные реального мира никогда не бывают только текстом. Инженерам, диагностирующим сбои серверов, нужно смотреть на температурные кривые, скриншоты устройств и логи вместе. Врачам при постановке диагнозов нужны снимки КТ или МРТ, отчёты об анализах и электронные медицинские карты одновременно. Традиционный текстовый RAG в лучшем случае может извлечь фразы вроде «температурная аномалия» или «подозрение на узел в лёгком», но он с трудом связывает эти описания с фактическим трендом на диаграмме или формой поражения на изображении и не может выполнять обратный поиск документов или знаний по изображениям, аудио или видео.

Мультимодальный RAG решает эту проблему неспособности разных модальностей «видеть» друг друга. Его ядро — кросс-модальное семантическое выравнивание. Система использует подходящие кодировщики для изображений, видео, аудио и текста вместе с OCR, ASR и анализом структуры, извлекает ключевую информацию из визуальных и аудиоисточников и отображает разные модальности в общее семантическое пространство, где можно построить единый мультимодальный индекс.

Во время поиска и генерации, спрашивает ли пользователь о диаграмме, показывающей пик продаж в Q3 2023, или загружает эскиз либо видео работы, система сначала находит ближайшие мультимодальные доказательства в этом едином пространстве, фильтрует их по таким сигналам, как текстовое сходство и сходство изображений, оставляет наиболее полезные фрагменты, а затем передаёт эти изображения, текстовые фрагменты и таблицы вместе мультимодальной LLM. Затем модель может ответить, объединяя доказательства из разных модальностей, и в идеале указать источник или подсветить релевантные области на изображении или в документе.

По сравнению с чисто текстовым RAG мультимодальный RAG может использовать больше видов доказательств и часто снижает галлюцинации, выдавая при этом более полные и более проверяемые ответы.
...
![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image14.png)

Представьте, что вы читаете статью Википедии о Берлине. Традиционный RAG сначала разрезал бы её на независимые абзацы, а затем встраивал бы каждый фрагмент. Если первое предложение гласит «Берлин — столица Германии», то более поздние фразы, такие как «город» или «его население», теряют связь с Берлином после разделения. Запрос вроде «Каково население Берлина?» может тогда не сработать, поскольку термин «Берлин» и информация о населении никогда не появлялись внутри одного фрагмента. Эта проблема становится ещё хуже для длинных документов. В страховом договоре на 200 страниц определение франшизы может появиться на странице 5, тогда как условия её применения — на странице 30. Разбиение по фиксированной длине может разделить эти связанные части на десятки изолированных фрагментов, и эксперименты показывают, что семантическое сходство при этом может резко обрушиваться.

Late Chunking переворачивает традиционный конвейер «сначала разбить, потом встроить» и вместо этого следует подходу «сначала встроить, потом разбить». С моделями эмбеддингов с длинным контекстом, способными обрабатывать порядка 8192 токенов, весь документ сначала пропускается через Transformer, давая эмбеддинги на уровне токенов, которые уже «увидели» весь документ. Только после этого эти глобально информированные эмбеддинги токенов агрегируются в эмбеддинги фрагментов согласно границам фрагментов. Получившиеся фрагменты больше не являются независимыми островами. Это контекстно-зависимые эмбеддинги, которые сохраняют межабзацные ссылки и концептуальные отношения.

На эталонных наборах данных BEIR Late Chunking широко превосходит традиционное разбиение, с особенно сильным выигрышем на более длинных документах. В сценариях с коротким текстом разница в основном исчезает, что подтверждает ключевое правило: чем длиннее документ, тем больше преимущество Late Chunking. Метод теперь интегрирован в Jina Embeddings v3. Хотя предварительное кодирование целого длинного документа может увеличить время вывода на 10–20 процентов, выигрыш в поиске в таких сценариях, как медицинские карты, юридические документы и технические руководства, может легко оправдать эту стоимость.

Late Chunking показывает, что модели эмбеддингов с длинным контекстом от 8K в этих сценариях — не избыточная инженерия. Они часто необходимы для получения высококачественных эмбеддингов фрагментов и представляют сдвиг от «сначала разбить, потом встроить» к «сначала встроить, потом разбить».

## 7.4 От RAG к RAG в эпоху агентов
...
RAG развился из инструмента генерации с дополнением через поиск в ключевую часть когнитивной архитектуры агента. Традиционный RAG построен на простом паттерне «спросить, найти, ответить» и принципиально пассивен. Он ждёт запроса и не действует проактивно. Чтобы прорваться сквозь эту пассивность и справляться с более сложными когнитивными задачами, RAG был глубоко объединён с возможностями агентов, породив новую парадигму: Agentic RAG.

В рамках этой парадигмы роль RAG фундаментально меняется. Он больше не только пассивный поставщик внешних знаний. Вместо этого он становится ядром обработки, поддерживающим интеллектуальное поведение под активным планированием, целеполаганием и саморефлексией агента. Это слияние придаёт всей системе целеориентированность, итеративную оптимизацию и автономное принятие решений, значительно углубляя качество взаимодействия человека и ИИ. Agentic RAG может понимать сложные задачи, декомпозировать их, планировать стратегии поиска и оценивать качество первоначальных результатов, чтобы решить, нужно ли более глубокое исследование.

![](../../../../ru-ru/stage-3/ai-advanced/rag-introduction/images/image15.png)

Ключ к этой способности — многослойный активный цикл. Столкнувшись со сложным запросом, агент сначала анализирует природу проблемы, разбивает её на подпроблемы и проектирует точные стратегии поиска для каждой подпроблемы. Получив первоначальные результаты, он оценивает их, судит, полна ли и релевантна ли информация, выявляет пробелы в знаниях и динамически генерирует более точные новые запросы. Этот итеративный процесс часто включает многошаговый поиск, где один раунд результатов открывает новые направления для следующего раунда, порождая цепочку исследования знаний, похожую на то, как работает исследователь-человек.

Чтобы поддерживать это непрерывное итеративное интеллектуальное поведение, особенно когда важны персонализация и долгосрочное накопление знаний, одного краткосрочного контекста разговора далеко не достаточно. Это приводит к потребности в долгосрочной структурированной памяти.

Именно поэтому RAG всё чаще наделяют ролью системы долгосрочной памяти агента и используют для построения полной архитектуры внешней памяти. Эта долгосрочная память дополняет краткосрочную память, которая отвечает за поддержание текущего контекста диалога. Система долгосрочной памяти опирается на три ключевых механизма:

1. Способность к структурированной индексации:
   Это позволяет агенту строить многомерные индексы над огромными объёмами неструктурированных данных — по времени, теме, отношениям сущностей и др., — поддерживая эффективный поиск с нескольких ракурсов, во многом подобно тому, как люди вспоминают информацию по разным подсказкам.
2. Интеллектуальное забывание:
   С помощью алгоритмов оценки ценности система может затухать или выборочно отбрасывать низкочастотную, слабо связанную или устаревшую информацию, сохраняя систему памяти компактной и эффективной и предотвращая перегрузку.
3. Консолидация знаний:
   Система перерабатывает разрозненный опыт диалогов и взаимодействий в структурированные знания. Через распознавание сущностей, извлечение отношений и семантическую кластеризацию фрагментированная информация связывается в графы знаний, превращая краткосрочный опыт в долгосрочные знания.

Эта система внешней памяти, построенная на RAG, не только значительно расширяет когнитивную границу агента, но и даёт ему способность продолжать обучение и эволюцию своих знаний. Она позволяет агенту накапливать опыт в ходе долгосрочного взаимодействия, формировать персонализированные паттерны работы и системы предметных знаний и поддерживать более сложные и более длительные задачи.

# Итоги

Retrieval-Augmented Generation — это не только технический метод компенсации галлюцинаций и устаревания знаний в больших моделях. Это также ключевой мост для превращения общей способности ИИ в глубокую корпоративную ценность. Эволюция от Naive RAG к модульным и агентным формам показывает, что каждая часть RAG должна непрерывно углубляться, включая более тонкую обработку данных, более научный выбор моделей на стадиях эмбеддингов, rerank и LLM и более систематическую оценку. Всё это — необходимые шаги к построению корпоративных систем знаний, которые управляемы, заслуживают доверия и эффективны. В то же время извлечение уроков из соревнований и инженерных кейсов — один из лучших способов углубить понимание технических деталей.

По мере того как Graph RAG, мультимодальное понимание и Late Chunking продолжают развиваться и сочетаться, RAG неуклонно выходит за старую границу «поиск и генерация» и движется к более глубокой семантической ассоциации и более устойчивой способности к памяти. Надежда в том, что эта статья обзорного характера поможет вам выстроить методологию полной цепочки — от принципа к практике и от оценки к эволюции, — чтобы в быстро меняющемся технологическом ландшафте вы могли строить высококачественные интеллектуальные приложения, которые действительно приземляются в реальном мире и способны справляться со сложными бизнес-вызовами.
